...
export default function SorteoWidget({
  onRegistroExitoso,
  allProducts = [],
  ...
  const getPremios = () => {
    if (sorteo?.premio) {
      ...
      return nombresPremios.map(nombre => {
        ...
        const productoReal = allProducts.find(
          p =>
        ...
        return {
          ...
          valor: "Sorpresa",
          img: PREMIOS_DEMO[Math.floor(Math.random() * PREMIOS_DEMO.length)]
            .img,
        };
      });
    }
  ...
  const handleParticipar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Intentar leer JSON de error primero, si existe
        let serverMessage: string | null = null;
        try {
          const json = await res.json();
          if (json && json.error) serverMessage = String(json.error);
        } catch (e) {
          try {
        ...
        if (res.status === 409) {
          setFormError(
            serverMessage ||
              "Este correo electrónico ya se encuentra participando en el sorteo."
          );
        } else {
          console.error(
  ...
  const nextPrize = () =>
    setCurrentPrize(prev => (prev + 1) % getPremios().length);
  const prevPrize = () =>
    setCurrentPrize(
      prev => (prev - 1 + getPremios().length) % getPremios().length
    );
      ...
      <div className="flex flex-col lg:flex-row min-h-[400px] md:min-h-[650px]">
        <div className="relative flex-1 bg-gradient-to-br from-[#fafafa] to-gray-50 flex flex-col p-4 sm:p-6 md:p-12 overflow-hidden border-r border-gray-100">
          ...
          <div className="flex-1 relative flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-10">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="text-left sm:text-center space-y-1 sm:space-y-2 flex-1">
                  <h3
                    className="text-lg sm:text-2xl md:text-3xl text-[#2d3748] leading-tight"
                    style={{ fontFamily: "var(--font-marcellus), serif" }}
                  >
                    {getPremios()[currentPrize].nombre}
                  </h3>
                  <p
                    className="text-[10px] sm:text-sm text-[#708090] italic line-clamp-1"
                    style={{ fontFamily: "var(--font-lora), serif" }}
                  >
                    {getPremios()[currentPrize].materiales}
                  </p>
                  <p
                    className="text-[9px] sm:text-[10px] font-black text-[#b76e79] uppercase tracking-widest pt-1"
                    style={{ fontFamily: "var(--font-poppins), sans-serif" }}
                  >
                    {getPremios()[currentPrize].valor}
                  </p>
                </div>
              </motion.div>
            </AnimatePresence>
            ...
            <button
              onClick={prevPrize}
              className="absolute left-0 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 bg-white/80 backdrop-blur rounded-full flex items-center justify-center text-gray-600 shadow-md hover:scale-110 transition-transform"
            >
              <ChevronLeft size={18} className="sm:w-5 sm:h-5" />
            </button>
            <button
              onClick={nextPrize}
              className="absolute right-0 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 bg-white/80 backdrop-blur rounded-full flex items-center justify-center text-gray-600 shadow-md hover:scale-110 transition-transform"
            >
            ...
            <AnimatePresence mode="wait">
              {ganador ? (
                ...
                >
                  ...
                  <p className="text-5xl font-serif text-[#b76e79] font-bold italic">
                    {ganador.nombre}
                  </p>
                </motion.div>
              ) : currentStep === 1 ? (
                ...
                >
                  <div className="space-y-3 sm:space-y-4">
                    ...
                    <h3
                      className="text-2xl sm:text-3xl text-[#2d3748]"
                      style={{ fontFamily: "var(--font-marcellus), serif" }}
                    >
                      Síguenos en Instagram
                    </h3>
                    <p className="text-xs sm:text-sm text-[#708090]">
                      Es obligatorio seguir nuestra cuenta para participar.
                    </p>
                  </div>
                  ...
                  <div className="space-y-3 py-4 sm:py-6 border-y border-gray-100">
                    <div className="relative flex justify-between items-center text-[8px] sm:text-[9px] font-black uppercase tracking-widest text-gray-400 z-10">
                      ...
                      <div className="flex flex-col items-center gap-2 text-[#b76e79] bg-white px-2">
                        <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-[#b76e79] text-white flex items-center justify-center shadow-md">
                          1
                        </div>
                        <span>Síguenos</span>
                      </div>
                      <div className="flex flex-col items-center gap-2 bg-white px-2">
                        <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gray-100 text-gray-400 flex items-center justify-center">
                          2
                        </div>
                        <span>Registro</span>
                      </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {!hasClickedInstagram && (
                      <p className="text-[9px] text-red-400 font-bold italic">
                        Haz clic en Instagram para continuar
                      </p>
                    )}
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    <div className="relative">
                      ...
                      <input
                        required
                        className="w-full pl-11 pr-4 py-3.5 bg-white border border-gray-100 focus:border-[#708090] rounded-xl outline-none text-xs"
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {["anillos", "collares", "aretes"].map(pref => (
                          <div
                            key={pref}
                            onClick={() =>
                              setFormData({ ...formData, preferencia: pref })
                            }
                            className={`cursor-pointer py-2 text-center rounded-lg border text-[9px] font-bold uppercase tracking-widest transition-all ${formData.preferencia === pref ? "bg-[#708090] text-white" : "bg-white text-gray-400"}`}
                          >
                            {pref}
                          </div>
                        ))}
                  ...
                  <button
                    disabled={loading}
                    type="submit"
                    className="w-full py-4 bg-[#708090] text-white rounded-2xl font-bold text-xs uppercase tracking-widest shadow-lg hover:bg-[#5a6a7a] transition-all"
                  >
                ...
                >
                  ...
                  <p className="text-xs sm:text-sm text-[#708090] font-medium">
                  ...
                  <button
                    onClick={() =>
                      window.scrollTo({
                        top:
                          document.getElementById("catalogo-estrategico")
                            ?.offsetTop || 0,
                        behavior: "smooth",
                      })
                    }
                    className="text-[10px] font-black text-[#708090] uppercase tracking-widest border-b border-[#708090]/20 pb-1"
                  >
                    Ver catálogo
                  </button>
                </motion.div>
